# BOB Solar — Agent Skills Reference

> This file defines the technical knowledge, patterns, and code conventions an AI coding agent MUST follow when working on this project.
...
import { pgTable, uuid, text, timestamp, decimal, integer, boolean, pgEnum } from 'drizzle-orm/pg-core';
...
import { getProjects, createProject, updateProject } from '@/actions/project-actions';
...
export const useQuoteTotals = () => useQuoteBuilderStore((s) => {
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          {company.logoUrl && <Image src={company.logoUrl} style={{ width: 80 }} />}
          <View>
...
export function calculateQuotation(input: {
  ...
  const subtotal = input.items.reduce((sum, item) => sum + Math.round(item.unitPrice * item.quantity), 0);
...
// Usage
export async function createCustomer(input: unknown): Promise<ActionResponse<Customer>> {
  try {
