ALTER TYPE "public"."project_status" ADD VALUE 'installation_completed' BEFORE 'completed';--> statement-breakpoint
...
ALTER TABLE "project_costs" ADD COLUMN "payment_method_id" uuid;--> statement-breakpoint
ALTER TABLE "project_costs" ADD CONSTRAINT "project_costs_payment_method_id_payment_methods_id_fk" FOREIGN KEY ("payment_method_id") REFERENCES "public"."payment_methods"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "project_costs_payment_method_id_idx" ON "project_costs" USING btree ("payment_method_id");