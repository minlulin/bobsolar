...
export default async function getNextConfig() {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    const withSerwist = withSerwistInit({
      swSrc: "src/sw.ts",
      swDest: "public/sw.js",
      // Provide an explicit revision so Serwist does not warn about missing
      // revisioning info for /offline.html (it is not part of Next.js's
      // built-asset manifest, so Serwist cannot auto-derive a hash for it).
      // Bump `revision` whenever the content of public/offline.html changes.
