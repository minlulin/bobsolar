...
describe("dashboard-actions", () => {
  ...
  it("returns finance quick view", async () => {
    vi.mocked(db.select)
      .mockImplementationOnce(() => selectChain([{ sum: "30000" }]) as never)
      .mockImplementationOnce(() => selectChain([{ sum: "10000" }]) as never)
      .mockImplementationOnce(() => selectChain([{ sum: "120000" }]) as never)
      .mockImplementationOnce(() => selectChain([{ sum: "50000" }]) as never)
      .mockImplementationOnce(() => selectChain([{ count: 2, amount: "40000" }]) as never);
