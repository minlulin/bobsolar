...
import { notificationTypeSchema } from "@/lib/domain/notification-types";
import {
...
export async function deleteAllNotifications(): Promise<ActionResponse<null>> {
  try {
    const auth = await requireAuth();
    ...
    return handleActionError(error, "deleteAllNotifications", "Failed to clear notifications");
  }
}
