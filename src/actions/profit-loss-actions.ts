...
export async function getProfitLossReport(
  ...
  try {
    ...
    const incomeAccounts = REVENUE_ACCOUNT_CODES;
    const expenseAccounts = OPERATING_EXPENSE_ACCOUNT_CODES;
    ...
    const cogsAccounts = COGS_ACCOUNT_CODES;
    ...
    const incomeItems: ProfitLossLineItem[] = incomeRows
      .filter(
        (row) =>
          isLedgerAccountCode(row.accountCode) &&
          (incomeAccounts as readonly string[]).includes(row.accountCode),
      )
      .map((row) => ({
    ...
    const cogsItems: ProfitLossLineItem[] = cogsRows
      .filter(
        (row) =>
          isLedgerAccountCode(row.accountCode) &&
          (cogsAccounts as readonly string[]).includes(row.accountCode),
      )
      .map((row) => ({
        accountCode: row.accountCode as LedgerAccountCode,
        accountName: row.accountName,
    ...
    const expenseItems: ProfitLossLineItem[] = expenseRows
      .filter(
        (row) =>
          isLedgerAccountCode(row.accountCode) &&
          (expenseAccounts as readonly string[]).includes(row.accountCode),
      )
      .map((row) => ({
        accountCode: row.accountCode as LedgerAccountCode,
        accountName: row.accountName,
