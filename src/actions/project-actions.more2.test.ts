    ...
    transaction: vi.fn(async (cb: (tx: any) => Promise<any>) => {
      ...
      const tx = {
        ...
        query: {
          ...
          projects: {
            findFirst: vi.fn(async () => state.quotation.project),
          },
        },
        select: vi.fn(() => {
          txSelectCall += 1;
          ...
          const selectedRows = (): unknown[] => {
            if (call === 1) return [];
            if (call === 2) return state.quoteItems;
            if (call % 2 === 1) return state.inventoryItem ? [state.inventoryItem] : [];
            return [{ total: state.txCostSum }];
          };
          // biome-ignore lint/suspicious/noExplicitAny: drizzle query chain mock
          const chain: any = {
            from: vi.fn(() => chain),
            ...
            orderBy: vi.fn(() => chain),
            limit: vi.fn(() => chain),
            for: vi.fn(async () => selectedRows()),
            // biome-ignore lint/suspicious/noThenProperty: thenable mock
            then: (resolve: (value: unknown) => unknown) =>
              Promise.resolve(resolve(selectedRows())),
          };
          return chain;
        }),
        update: vi.fn(() => ({
          set: vi.fn((values: unknown) => {
            state.stockUpdates.push(values);
        ...
        insert: vi.fn(() => ({
          values: vi.fn((values: unknown) => {
            txInsertCall += 1;
            if (txInsertCall > 1) state.projectCostInsertValues.push(values);
            return {
              returning: vi.fn(async () =>
                txInsertCall === 1
                  ? [state.projectInsertRow]
                  : [{ id: `cost-${txInsertCall - 1}` }],
              ),
            };
          }),
        })),
