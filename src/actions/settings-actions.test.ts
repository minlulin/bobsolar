...
vi.mock("@/lib/db", () => {
  const db = {
    ...
    insert: vi.fn(() => ({
      values: vi.fn((payload: MockInsertPayload) => {
    ...
    transaction: vi.fn(async (cb: (tx: MockTx) => Promise<unknown>) => {
      const tx: MockTx = {
        select: vi.fn(() => ({
          from: vi.fn(async () => [{ count: state.users.length }]),
        })),
        insert: vi.fn(() => ({
          values: vi.fn((payload: MockInsertPayload) => {
            spies.insertUserValues(payload);
...
describe("settings-actions", () => {
  ...
  it("setCompanyLogoUrl validates and revalidates settings cache tags", async () => {
  ...
  it("updateCompanySettings filters out non-standard setting keys", async () => {
    ...
    expect(spies.insertCompanyValues).toHaveBeenCalledWith([
      ...
      { key: "company_phone", value: "0912345678" },
      { key: "company_email", value: "ops@example.com" },
    ]);
  });
});
