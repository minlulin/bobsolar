...
import { companySettingsSchema } from "@/lib/validators/settings";
...
export async function getCompanySettings(): Promise<ActionResponse<Record<string, string>>> {
  ...
  try {
    ...
    return handleActionError(error, "getCompanySettings", "Failed to load settings");
  }
}
...
export async function updateCompanySettings(
  ...
  try {
    ...
    if (validatedEntries.length > 0) {
      await db
        .insert(companySettings)
        .values(validatedEntries.map(([key, value]) => ({ key, value: value ?? "" })))
        .onConflictDoUpdate({
          target: companySettings.key,
...
export async function getSettingsUsers(): Promise<
  ActionResponse<{ isAdmin: boolean; users: SafeUser[]; me: SafeUser | null }>
> {
  ...
  try {
    ...
    // Only select safe columns — never expose passwordHash.
...
export async function createSettingsUser(raw: unknown): Promise<ActionResponse<null>> {
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      if (existingCount && existingCount.count >= USER_CAP) {
        throw new Error(`User limit reached (${USER_CAP} users max)`);
      }
