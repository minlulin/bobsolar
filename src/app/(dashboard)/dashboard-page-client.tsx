  ...
  return (
    ...
    >
      ...
      >
        ...
        <motion.div
          aria-hidden="true"
          className="absolute -right-12 -top-16 h-40 w-40 rounded-full bg-amber-300/15 blur-3xl"
        ...
        <div className="relative flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.24em] text-amber-300/90">
              Command Center
            </p>
      ...
      <motion.section variants={item} className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
        <StatCard title={statMeta[0].title} value={formatMMK(stats?.totalRevenue ?? 0)} hint={`MoM ${stats?.revenueTrendPercent ?? 0}%`} loading={statsQuery.isLoading} tone={statMeta[0].tone} />
        <StatCard title={statMeta[1].title} value={String(stats?.activeProjectsCount ?? 0)} hint="Planning + In progress + On hold" loading={statsQuery.isLoading} tone={statMeta[1].tone} />
        <StatCard title={statMeta[2].title} value={String(stats?.pendingQuotationsCount ?? 0)} hint="Draft + Sent" loading={statsQuery.isLoading} tone={statMeta[2].tone} />
        <StatCard title={statMeta[3].title} value={String(stats?.acceptedThisMonth ?? 0)} hint="Current month" loading={statsQuery.isLoading} tone={statMeta[3].tone} />
        <StatCard title={statMeta[4].title} value={String(stats?.totalCustomers ?? 0)} hint="Active records" loading={statsQuery.isLoading} tone={statMeta[4].tone} />
        <StatCard title={statMeta[5].title} value={String(stats?.overdueAlertsCount ?? 0)} hint={`Conversion ${stats?.quotationConversionRate ?? 0}%`} loading={statsQuery.isLoading} tone={statMeta[5].tone} />
      </motion.section>
        ...
        <Card title="Upcoming Alerts">
          <div className="space-y-3">
            ...
            {alerts.map((alert) => (
              ...
              >
              <Link href="/warranty" className="group flex items-start justify-between rounded-xl border border-white/10 bg-white/[0.025] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] transition-colors hover:border-white/20 hover:bg-white/[0.06]">
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'pointer-events-none absolute -right-8 -top-10 h-28 w-28 rounded-full bg-gradient-to-br opacity-15 blur-2xl transition-opacity duration-500 group-hover:opacity-35',
          tone,
        )}
      />
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute -right-6 -top-6 h-16 w-16 rounded-full bg-amber-300/10 blur-xl transition-opacity duration-500 group-hover:opacity-100" />
