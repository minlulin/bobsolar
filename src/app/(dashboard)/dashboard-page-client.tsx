...
export default function DashboardPage(): React.JSX.Element {
  ...
  const financeError = financeQuery.isError;
  ...
  return (
    ...
    >
      ...
      <motion.div
        variants={item}
        className="surface-panel col-span-12 flex flex-col justify-between gap-4 rounded-2xl border p-6 sm:flex-row sm:items-center sm:p-8"
      >
        <div className="space-y-1">
          ...
          {statsError ? (
            <p className="text-destructive text-xs font-medium">
              Stats are temporarily unavailable. Retry in a moment.
            </p>
          ) : null}
        </div>
      ...
      <motion.div variants={item} className="col-span-12 xl:col-span-7">
        {statsError ? (
          <SectionError message="Dashboard stats failed to load. Other sections remain available." />
        ) : (
      ...
      <motion.div variants={item} className="col-span-12 xl:col-span-7">
        <SectionCard title="Finance Quick View">
          {financeError ? (
            <SectionError message="Finance quick view is unavailable. Open Finance Dashboard for details." />
          ) : (
            <>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <MiniStatCard
                  ...
                  icon={DollarSign}
                  color={
                    (finance?.monthNetMovement ?? 0) >= 0 ? "text-emerald-600" : "text-rose-600"
                  }
      ...
      <motion.div variants={item} className="col-span-12 xl:col-span-5">
        <SectionCard title="Priority Signals">
          <div className="space-y-3">
            {alertsError && (
              <SectionError message="Signals failed to load. You can still open Warranty page." />
            )}
            ...
            {!alertsQuery.isPending && !alertsError && alerts.length === 0 && (
...
function SectionCard({
  ...
  return (
    <div className="surface-panel flex h-full flex-col rounded-2xl border p-5 sm:p-6">
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <div className="surface-panel group hover:border-primary/30 relative flex min-h-36 flex-col justify-between overflow-hidden rounded-2xl border p-5 transition-colors sm:p-6">
          <div className={cn("absolute inset-y-5 left-0 w-1 rounded-r-full", tone)} />
          <div>
...
function QuickAction({
  ...
  return (
    <Button
      ...
      className={cn(
        "h-11 w-full justify-start gap-3 rounded-xl px-4 font-bold transition-all",
        primary && "solar-cta",
...
function QuickCommandLink({
  ...
  return (
    <Link
      href={href}
      className="hover:bg-muted/45 focus-visible:border-ring focus-visible:ring-ring/40 group flex min-h-10 items-center gap-3 rounded-xl px-2.5 py-1.5 transition-colors outline-none focus-visible:ring-2"
    >
      <div className="bg-muted text-muted-foreground group-hover:bg-solar/15 group-hover:text-foreground flex h-7 w-7 shrink-0 items-center justify-center rounded-lg transition-colors">
        {icon}
      </div>
      <p className="text-foreground min-w-0 flex-1 text-sm font-semibold">{title}</p>
