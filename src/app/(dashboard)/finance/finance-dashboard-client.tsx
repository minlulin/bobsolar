...
interface FinanceDashboardClientProps {
  ...
  initialRisk: ReceivableRiskProject[];
        ...
        <Card className="border-border">
          ...
          <CardContent>
            {isLoadingRisk ? (
              ...
              <div className="space-y-3">
                {risk.slice(0, 5).map((project) => (
                  <RiskProjectRow key={project.projectId} project={project} />
                ))}
                {risk.length > 5 && (
...
function AssetCard({ label, value, icon: Icon, isLoading }: AssetCardProps): React.JSX.Element {
  return (
    <Card className="border-border transition-shadow hover:shadow-sm">
...
interface RiskProjectRowProps {
  project: ReceivableRiskProject;
}

function RiskProjectRow({ project }: RiskProjectRowProps): React.JSX.Element {
  const isOverdue = project.daysOverdue > 0;

  return (
    <div className="border-border flex items-center justify-between rounded-lg border p-3">
      <div className="flex-1">
        <div className="flex items-center gap-2">
          <span className="text-foreground text-sm font-medium">{project.projectNumber}</span>
          {isOverdue && (
