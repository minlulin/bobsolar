...
function statusBadgeTone(status: ProjectStatus) {
  switch (status) {
    ...
    case 'completed':
      return 'border-white/35 bg-emerald-500/15 text-emerald-200';
    case 'cancelled':
  ...
  return (
    <div className="space-y-8 pb-32">
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        <TabsList className="flex h-auto flex-wrap gap-3 rounded-[2rem] bg-white/10 p-3">
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid gap-4 md:grid-cols-3">
            {[
              ...
              <div
                key={card.label}
                className="border-border rounded-3xl border bg-white/[0.04] px-7 py-6 backdrop-blur"
              >
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <motion.div variants={staggerContainer} animate="animate">
              <div className="space-y-5 rounded-[2rem] border border-white/15 bg-black/45 p-6">
        ...
        <TabsContent value="remarks" className="space-y-6">
          {canEditOperational ? (
            <form
              className="space-y-6 rounded-[2rem] border border-white/15 bg-black/45 p-6"
          ...
          <ScrollArea className="h-[520px] rounded-[2rem] border border-white/10 bg-black/40 p-6">
          ...
          >
            {proj.warrantyAlerts.map((alert) => (
              <div
                key={alert.id}
                className={cn(
                  'rounded-[1.75rem] border bg-black/50 p-6 backdrop-blur',
                  alert.isResolved
...
function ProjectOperationalNotes({
  ...
  return (
    <div className="space-y-5 rounded-[2rem] border border-white/15 bg-black/35 p-6">
      <h3 className="text-muted-foreground text-[10px] font-bold uppercase">
        Operational notes
      </h3>
      <Textarea
