...
export function QuotationsGridClient({
  ...
  const {
    ...
    isError,
    error,
  } = useQuotations(queryFilters, isDefault ? initialData : undefined);
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="border-destructive/50 bg-destructive/5 flex flex-col items-center justify-center rounded-2xl border py-24 text-center">
          ...
          <h3 className="text-xl font-semibold text-destructive">Failed to load quotations</h3>
          <p className="text-muted-foreground mt-2 max-w-xs">
            {error?.message ?? "An unexpected error occurred while fetching quotations."}
          </p>
          <Button
            variant="outline"
            className="mt-4"
