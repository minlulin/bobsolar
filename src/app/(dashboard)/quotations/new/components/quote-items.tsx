...
import { GripVertical, Hash, Lock, Trash2 } from "lucide-react";
...
function ItemRow({ item, index }: { item: QuoteBuilderItem; index: number }): React.JSX.Element {
  ...
  // Catalog items (from inventory) have a non-null itemId — lock their price & description
  const isCatalogItem = item.itemId !== null;
  ...
  return (
    ...
    >
      ...
      {/* Description — locked for catalog items */}
      <div>
        {isCatalogItem ? (
        ...
        {!isCatalogItem && (
          <Badge variant="secondary" className="mt-1 text-[9px] font-bold uppercase tracking-wider">
            Custom Service
          </Badge>
        )}
      </div>
      ...
      {/* Unit Price — locked for catalog items */}
      <div>
        {isCatalogItem ? (
          <TooltipProvider delayDuration={200}>
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex cursor-not-allowed items-center justify-end gap-1.5 rounded-lg px-2 py-2">
              ...
              <TooltipContent side="top" className="max-w-[200px] text-center text-xs">
                Catalog price is locked to prevent pricing errors. Update the price in Inventory
