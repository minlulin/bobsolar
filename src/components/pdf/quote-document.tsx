...
// Register Inter font using local bundled fonts.  The public/fonts/ directory
// is available at `process.cwd()` during local dev / `next start` but is NOT
// included in Vercel serverless function bundles (Vercel serves static assets
// via CDN instead).  A try-catch ensures PDF rendering falls back to built-in
// fonts (Helvetica) when the font files are absent on Vercel.
try {
  ...
  Font.register({
    ...
    fonts: [
      {
        // fs.readFileSync returns a Buffer which react-pdf accepts at runtime
        // (the type declaration only lists string, but Buffer/Uint8Array works).
        src: fs.readFileSync(
          path.join(fontDir, 'inter-regular.woff2'),
        ) as unknown as string,
        fontWeight: 400,
      },
      {
        src: fs.readFileSync(
          path.join(fontDir, 'inter-semibold.woff2'),
        ) as unknown as string,
        fontWeight: 600,
      },
      {
        src: fs.readFileSync(
          path.join(fontDir, 'inter-bold.woff2'),
        ) as unknown as string,
        fontWeight: 700,
      },
    ],
  ...
  // Font files not reachable (e.g. Vercel serverless) — the PDF will render
  // with the default built-in font (Helvetica).
}
